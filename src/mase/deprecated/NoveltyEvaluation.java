...
public class NoveltyEvaluation implements PostEvaluator {
    ...
    protected List<List<Integer>> behavioursAge;
    ...
    public static final String P_ARCHIVE_MATURATION = "ns-archive-maturation";
    ...
    protected int archiveMaturation;
    ...
    public void setup(EvolutionState state, Parameter base) {
        ...
        this.archiveMaturation = state.parameters.getInt(base.push(P_ARCHIVE_MATURATION), null);
        ...
        this.behavioursAge = new ArrayList<List<Integer>>(nPops);
        if (archiveMode == V_NONE) {
            for (int i = 0; i < nPops; i++) {
                archives.add(Collections.EMPTY_LIST);
                behavioursAge.add(Collections.EMPTY_LIST);
            }
        } else if (archiveMode == V_SHARED) {
            ArrayList<BehaviourResult> arch = new ArrayList<BehaviourResult>(sizeLimit);
            ArrayList<Integer> age = new ArrayList<Integer>(sizeLimit);
            for (int i = 0; i < nPops; i++) {
                archives.add(arch);
                behavioursAge.add(age);
            }
        } else if (archiveMode == V_MULTIPLE) {
            for (int i = 0; i < nPops; i++) {
                archives.add(new ArrayList<BehaviourResult>(sizeLimit));
                behavioursAge.add(new ArrayList<Integer>(sizeLimit));
            }
        ...
        for (int p = 0; p < pop.subpops.size(); p++) {
            ...
            for (int j = 0; j < pop.subpops.get(p).individuals.size(); j++) {
                ...
                for (int i = 0; i < archive.size(); i++) {
                    if (behavioursAge.get(p).get(i) < state.generation - archiveMaturation) {
                        BehaviourResult er = archive.get(i);
                        BehaviourResult erInd = (BehaviourResult) indFit.getNoveltyBehaviour();
                        distances.add(Pair.of(distance(er, erInd), true));
                    }
                }
                ...
                for (int j = 0; j < pop.subpops.get(i).individuals.size(); j++) {
                    ...
                    if (state.random[0].nextDouble() < addProb) {
                        ...
                        if (archive.size() == sizeLimit) {
                            ...
                            behavioursAge.get(i).set(index, state.generation);
                        } else {
                            archive.add(br);
                            behavioursAge.get(i).add(state.generation);
                        }
                    }
                }
            }
