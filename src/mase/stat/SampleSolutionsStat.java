...
public class SampleSolutionsStat extends Statistics {
    ...
    public void postEvaluationStatistics(EvolutionState state) {
        ...
        while (added < sampleSize) {
            int sub = state.random[0].nextInt(state.population.subpops.size());
            int ind = state.random[0].nextInt(state.population.subpops.get(sub).individuals.size());
        ...
        for (int i = 0; i < sampleSize; i++) {
            Individual ind = state.population.subpops.get(subs[i]).individuals.get(inds[i]);
